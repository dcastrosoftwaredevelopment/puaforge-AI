    ...
    setup(build) {
      ...
      build.onResolve({ filter: /^\./ }, (args) => {
        ...
        for (const ext of extensions) {
          ...
          if (files[candidate]) return { path: candidate, namespace: 'virtual' };
        }
        for (const ext of extensions) {
...
export async function publish(req: Request<object, object, PublishBody>, res: Response) {
  ...
  try {
    ...
    for (const [path, code] of Object.entries(files)) {
      if (path !== '/index.html') projectFiles[path] = code;
    }
...
`;
    ...
    for (const pkg of allPkgs) importMap[pkg] = `https://esm.sh/${pkg}`;
...
${bundledJs}
    ...
    if (project?.customDomain) invalidateSiteCache(project.customDomain);
