...
export function generateImagesFiles(images: ProjectImage[]): Record<string, string> {
  ...
  const cssVars = images.map((img) => `  ${toCssVarName(img.name)}: url('${img.dataUrl ?? img.url}');`).join('\n');
...
export function useProjectImages() {
  ...
  const syncImagesFiles = useCallback(
    (updatedImages: ProjectImage[]) => {
      setFiles((prev) => {
        const next = { ...prev };
  ...
  const addImage = useCallback(
    async (file: File) => {
      ...
      const uploaded = await withPlanLimit(() =>
        api.upload<ProjectImage>(`/api/projects/${activeProjectId}/images`, formData, authHeaders),
      );
      if (!uploaded) return;
