...
const inputCls =
  'w-full text-[11px] bg-bg-elevated border border-border-subtle rounded px-1.5 py-1 text-text-secondary outline-none focus:border-forge-terracotta font-mono';
    ...
    <StyleEditorSection title={t('inspect.sectionDimensions')}>
      <StyleEditorRow label={t('inspect.width')}>
        <input
          key={`${elementId}-w`}
          type="text"
          defaultValue={fromTailwindValue(width)}
          onChange={(e) =>
            withDebounce('w', () =>
              e.target.value ? applyLiveClass(`w-${toTailwindValue(e.target.value)}`) : removeLiveCategory('w-0'),
            )
          }
          onBlur={() => {
            flushDebounce('w');
      ...
      <StyleEditorRow label={t('inspect.height')}>
        <input
          key={`${elementId}-h`}
          type="text"
          defaultValue={fromTailwindValue(height)}
          onChange={(e) =>
            withDebounce('h', () =>
              e.target.value ? applyLiveClass(`h-${toTailwindValue(e.target.value)}`) : removeLiveCategory('h-0'),
            )
          }
          onBlur={() => {
            flushDebounce('h');
