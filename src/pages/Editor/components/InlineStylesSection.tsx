...
const InlineStylesSection = memo(() => {
  ...
  const {
    ...
    removeInlineProp,
    addInlineProp,
  } = useInlineStylesSection();
  const [newProp, setNewProp] = useState('');
  const [newValue, setNewValue] = useState('');
  const inputCls =
    'text-[11px] bg-bg-elevated border border-border-subtle rounded px-1.5 py-1 text-text-secondary outline-none focus:border-forge-terracotta font-mono';
  return (
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter' && newProp.trim()) {
              addInlineProp(newProp, newValue);
              setNewProp('');
              setNewValue('');
            }
          }}
