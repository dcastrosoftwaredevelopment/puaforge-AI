...
<html lang="en">
  ...
  <body>
    <div id="root"></div>
    <script>
(function(){var _a=false,_o=null,_c=0;function uid(){return'v'+(++_c)+'_'+Math.random().toString(36).slice(2,6)}function assignIds(){document.querySelectorAll('*').forEach(function(e){if(!e.hasAttribute('data-vibe-id'))e.setAttribute('data-vibe-id',uid())})}function removeIds(){document.querySelectorAll('[data-vibe-id]').forEach(function(e){e.removeAttribute('data-vibe-id')})}function domNode(e,d){if(d>12)return null;var t=e.tagName.toLowerCase();if('script style head link meta'.indexOf(t)!==-1)return null;var k=[];for(var i=0;i<e.children.length;i++){var n=domNode(e.children[i],d+1);if(n)k.push(n)}return{id:e.getAttribute('data-vibe-id')||'',tagName:t,className:e.getAttribute('class')||'',children:k}}function sendTree(){var r=document.getElementById('root');if(!r)return;var t=[];for(var i=0;i<r.children.length;i++){var n=domNode(r.children[i],0);if(n)t.push(n)}window.parent.postMessage({type:'VIBE_DOM_TREE',tree:t},'*')}function info(e){var r=e.getBoundingClientRect();return{id:e.getAttribute('data-vibe-id')||'',tagName:e.tagName.toLowerCase(),className:e.getAttribute('class')||'',rect:{top:r.top,left:r.left,width:r.width,height:r.height}}}function hit(x,y){if(_o)_o.style.pointerEvents='none';var e=document.elementFromPoint(x,y);if(_o)_o.style.pointerEvents='all';return(e&&e!==document.body&&e!==document.documentElement)?e:null}function onMove(e){var el=hit(e.clientX,e.clientY);if(el)window.parent.postMessage(Object.assign({type:'VIBE_ELEMENT_HOVERED'},info(el)),'*')}function onClick(e){e.preventDefault();e.stopPropagation();var el=hit(e.clientX,e.clientY);if(el)window.parent.postMessage(Object.assign({type:'VIBE_ELEMENT_SELECTED'},info(el)),'*')}function activate(){assignIds();sendTree();if(!_o){_o=document.createElement('div');_o.style.cssText='position:fixed;inset:0;z-index:9999;cursor:crosshair;';_o.addEventListener('mousemove',onMove);_o.addEventListener('click',onClick);document.body.appendChild(_o)}}function deactivate(){if(_o){_o.removeEventListener('mousemove',onMove);_o.removeEventListener('click',onClick);_o.remove();_o=null}removeIds()}window.addEventListener('message',function(e){if(!e.data||typeof e.data!=='object')return;var t=e.data.type;if(t==='VIBE_INSPECT_TOGGLE'){var on=!!e.data.enabled;if(on===_a)return;_a=on;on?activate():deactivate()}else if(t==='VIBE_SELECT_BY_ID'){var el=document.querySelector('[data-vibe-id="'+e.data.id+'"]');if(!el)return;try{el.scrollIntoView({behavior:'smooth',block:'nearest'})}catch(x){}window.parent.postMessage(Object.assign({type:'VIBE_ELEMENT_SELECTED'},info(el)),'*')}else if(t==='VIBE_HOVER_BY_ID'){var el2=document.querySelector('[data-vibe-id="'+e.data.id+'"]');if(el2)window.parent.postMessage(Object.assign({type:'VIBE_ELEMENT_HOVERED'},info(el2)),'*')}else if(t==='VIBE_REFRESH_TREE'){if(_a){assignIds();sendTree()}}})})();
    </script>
  </body>
</html>`
