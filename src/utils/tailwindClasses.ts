...
function expandShorthands(classes: string[]): Map<string, string> {
  ...
  for (const cls of classes) {
    if (/^p-/.test(cls)) {
      const v = cls.slice(2);
      ['pt', 'pr', 'pb', 'pl'].forEach((s) => sides.set(s, v));
    } else if (/^px-/.test(cls)) {
      const v = cls.slice(3);
      ['pr', 'pl'].forEach((s) => sides.set(s, v));
    } else if (/^py-/.test(cls)) {
      const v = cls.slice(3);
      ['pt', 'pb'].forEach((s) => sides.set(s, v));
    } else if (/^m-/.test(cls)) {
      const v = cls.slice(2);
      ['mt', 'mr', 'mb', 'ml'].forEach((s) => sides.set(s, v));
    } else if (/^mx-/.test(cls)) {
      const v = cls.slice(3);
      ['mr', 'ml'].forEach((s) => sides.set(s, v));
    } else if (/^my-/.test(cls)) {
      const v = cls.slice(3);
      ['mt', 'mb'].forEach((s) => sides.set(s, v));
...
export function parseClasses(className: string): ParsedClasses {
  ...
  for (const cls of classes) {
    ...
    if (FONT_SIZE_SET.has(cls)) {
      parsed.fontSize = cls;
      continue;
    }
    if (FONT_WEIGHT_SET.has(cls)) {
      parsed.fontWeight = cls;
      continue;
    }
    if (TEXT_ALIGN_SET.has(cls)) {
      parsed.textAlign = cls;
      continue;
    }
    if (DISPLAY_SET.has(cls)) {
      parsed.display = cls;
      continue;
    }
    if (FLEX_DIR_SET.has(cls)) {
      parsed.flexDir = cls;
      continue;
    }
    if (JUSTIFY_SET.has(cls)) {
      parsed.justify = cls;
      continue;
    }
    if (ALIGN_SET.has(cls)) {
      parsed.alignItems = cls;
      continue;
    }
    if (OVERFLOW_SET.has(cls)) {
      parsed.overflow = cls;
      continue;
    }
    if (ROUNDED_SET.has(cls)) {
      parsed.rounded = cls;
      continue;
    }
    if (SHADOW_SET.has(cls)) {
      parsed.shadow = cls;
      continue;
    }
    if (BORDER_WIDTH_SET.has(cls)) {
      parsed.borderWidth = cls;
      continue;
    }
    if (/^w-/.test(cls)) {
      parsed.width = getSpacingVal(cls, 'w-');
      continue;
    }
    if (/^h-/.test(cls)) {
      parsed.height = getSpacingVal(cls, 'h-');
      continue;
    }
    if (/^max-w-/.test(cls)) {
      parsed.maxWidth = getSpacingVal(cls, 'max-w-');
      continue;
    }
    if (/^gap-/.test(cls)) {
      parsed.gap = getSpacingVal(cls, 'gap-');
      continue;
    }
    if (/^opacity-/.test(cls)) {
      parsed.opacity = getSpacingVal(cls, 'opacity-');
      continue;
    }
    if (/^text-/.test(cls)) {
      parsed.textColor = cls;
      continue;
    } // after size/align checks
    if (/^bg-/.test(cls)) {
      parsed.bgColor = cls;
      continue;
    }
    if (/^border-[A-Za-z]/.test(cls)) {
...
export function removeClassCategory(className: string, representative: string): string {
  ...
  return className
    .split(/\s+/)
    .filter(Boolean)
    .filter((c) => !sameCategory(c, representative))
    .join(' ');
}
